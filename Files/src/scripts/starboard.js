...
client.on('messageReactionAdd', async (reaction, user) => {
	...
	if (message.author.id === user.id) return message.lineReplyNoMention(`${user}, you can't star your own messages.`);
	if (message.author.bot) return message.lineReplyNoMention(`${user}, you can't star bot messages!`);
	const starChannel = message.guild.channels.cache.get(channel)
	if (!starChannel) return message.lineReplyNoMention(`It appears that you do not have a \`${starboardChannel}\` channel.`); 
	...
	if (!stars) {
		const image = message.attachments.size > 0 ? await extension(reaction, message.attachments.array()[0].url) : '';
		if (image === '' && message.cleanContent.length < 1) return message.lineReplyNoMention(`${user}, you cannot star an empty message.`);
		const embed = new MessageEmbed()
			.setColor(15844367)
